...
{
	return findPolysAroundCircle(centerRef, centerPos, radius, filter, resultRef, resultParent, resultCost, maxResult, m_nodePool, m_openList);
}
...
int	dtNavMesh::findPolysAroundShape(dtPolyRef centerRef, const float* verts, const int nverts,
									...
									const int maxResult) const
{
	return findPolysAroundShape(centerRef, verts, nverts, filter, resultRef, resultParent, resultCost, maxResult, m_nodePool, m_openList);
}
